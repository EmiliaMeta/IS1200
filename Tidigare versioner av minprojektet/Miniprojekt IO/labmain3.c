...
int timeoutcount = 0;
int old_sw9_state = 0; // för kantdetektering
...
void handle_interrupt(unsigned int cause) {
    ...
    // sw9
    if (*sw_ipending & (1u << 9)) {
        *sw_ipending = (1u << 9);  // ack direkt

        int sw9_state = (*sw_data & (1u << 9)) ? 1 : 0;

        if (sw9_state && !old_sw9_state) {  
            // knapptryckning (rising edge)
            mytime += 2;
            update_displays();
        }

        old_sw9_state = sw9_state; // uppdatera senaste läget
    }
}
