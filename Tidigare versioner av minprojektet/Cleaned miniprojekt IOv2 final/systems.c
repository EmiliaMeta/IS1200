...
int fightsys(Player *player, Player *enemy, int is_final_boss) {
    ...
    while (1) {
        ...
        switch (action) {
            ...
            case 3: {  // Use item
                print_and_wait("Items cannot be used in battle right now.", player);
                break;
            }
        ...
        if (p->health <= 0) {
            ...
            if (any_alive) {
                ...
                if (respawn_room < 1) respawn_room = 1;
