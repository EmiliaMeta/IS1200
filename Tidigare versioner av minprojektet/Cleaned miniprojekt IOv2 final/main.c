/**
 * Initializes the DTEK-V hardware, player, and game systems,
 * then starts the map traversal and gameplay loop.
 */

#include "objects.h"
#include "menu.h"
#include "map.h"
#include "io.h"
#include "random.h"

// Global spelare, används av interrupt-funktioner
Player global_player;

// Främsta spelstart funktioner (definierade i systems.c)
void shopsys(Player *player, Item *item);
void catchsys(Player *player, Item *item);
void fightsys(Player *player, Player *enemy, int is_final_boss);

int main(void) {
    // Initierar I/O och interrupts
    io_init();
    enable_interrupt();

    // Initierar slumpgeneratorn med ett statiskt seed
    srand_custom(12345);

    // Skapar spelare med startmynt
    Player player = create_player(10);

    // Initierar kartan
    Room map[NUM_ROOMS];
    init_map(map);

    // Startar spelrörelse (main gameplay loop)
    move_player(&player, map);

    // Returnerar aldrig hit under normalt spel
    return 0;
}
