...
void print_sieves(int n){ 
    //Deklarera pekare A till char på heapen.
    //*A är en char.
    //malloc reserverar minne på heapen, n+1 antal bytes och varje element får ha storlek char.
    char *A = malloc((n+1) * sizeof *A); 
   
    //Initiera alla värden till sanna förutom 0 och 1.
    for (int i = 0; i <= n; i++){
        A[i] = 1; //sann
    }
    A[0] = 0; // falsk
